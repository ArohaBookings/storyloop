...
-- Reads public.stripe_webhook_events, the table production uses (rewritten
-- 2026-09-23 from the private table before first being applied).
...
as $$
  with window_events as (
    select event_id, type, status, received_at, completed_at, error
    from public.stripe_webhook_events
    where received_at >= now() - make_interval(days => greatest(1, least(p_days, 90)))
  )
  select jsonb_build_object(
    ...
    'webhooks', jsonb_build_object(
      ...
      'stuck_processing', (
        ...
        where status = 'processing' and received_at < now() - interval '15 minutes'
      ),
      'by_type', coalesce((
      ...
      'recent_failures', coalesce((
        select jsonb_agg(jsonb_build_object('type', type, 'status', status, 'at', at, 'error', left(coalesce(error, ''), 240)) order by at desc)
        from (
          select type, status, coalesce(completed_at, received_at) as at, error
          from public.stripe_webhook_events
          where status = 'failed'
             or (status = 'processing' and received_at < now() - interval '15 minutes')
          order by coalesce(completed_at, received_at) desc
          limit 20
        ) f
      ), '[]'::jsonb)
    )
