...
begin
  ...
  r := public.begin_stripe_webhook_event('evt_new', 'invoice.paid');
  assert r = 'skip', format('2 retry while processing: expected skip, got %s', r);

  update public.stripe_webhook_events set status = 'processed' where event_id = 'evt_new';
  r := public.begin_stripe_webhook_event('evt_new', 'invoice.paid');
  assert r = 'skip', format('3 completed event: expected skip, got %s', r);

  insert into public.stripe_webhook_events (event_id, type, status, received_at, error) values ('evt_failed', 'invoice.paid', 'failed', now(), 'boom');
  ...
  assert (select error from public.stripe_webhook_events where event_id = 'evt_failed') is null, '4 error not cleared';

  -- The fix: a handler killed mid-run left this claimed for 20 minutes.
  insert into public.stripe_webhook_events (event_id, type, status, received_at) values ('evt_stuck', 'customer.subscription.deleted', 'processing', now() - interval '20 minutes');
  ...
  r := public.begin_stripe_webhook_event('evt_stuck', 'customer.subscription.deleted');
  assert r = 'skip', format('6 just reclaimed: expected skip, got %s', r);

  -- A handler that may genuinely still be running is never double-claimed.
  insert into public.stripe_webhook_events (event_id, type, status, received_at) values ('evt_slow', 'invoice.paid', 'processing', now() - interval '10 minutes');
  r := public.begin_stripe_webhook_event('evt_slow', 'invoice.paid');
  assert r = 'skip', format('7 ten-minute claim left alone: expected skip, got %s', r);
  ...
  update public.stripe_webhook_events set received_at = now() - interval '16 minutes' where event_id = 'evt_slow';
